#!/usr/bin/env bash
# demo-status.sh — Reset tests, start status server, run tests with staggered delays
#
# Shows the status detail page (http://localhost:4740/status) auto-updating via SSE
# as tests complete one by one.
#
# Usage:
#   bash scripts/demo-status.sh
#   # Then open http://localhost:4740/status in a browser or OBS capture
...
# Start status server
echo "Starting status server on http://localhost:4740/status ..."
...
echo "=== Open http://localhost:4740/status to watch live updates ==="
echo ""
...
echo "=== Server still running at http://localhost:4740/status (kill with: kill $SERVER_PID) ==="
